      ...
      <div className="border-t py-12 [border-image:linear-gradient(to_right,transparent,--theme(--color-slate-400/.25),transparent)1] md:py-20">
        ...
        <div className="mx-auto max-w-3xl pb-12 text-center">
          <h2 className="animate-[gradient_6s_linear_infinite] bg-[linear-gradient(to_right,var(--color-gray-200),var(--color-indigo-200),var(--color-gray-50),var(--color-indigo-300),var(--color-gray-200))] bg-[length:200%_auto] bg-clip-text pb-4 font-nacelle text-3xl font-semibold text-transparent md:text-4xl">
            Cashy Link مناسب لمين؟
          </h2>
          <p className="text-lg text-indigo-200/65">
            مصمم لأصحاب المحلات والتجار اللي محتاجين طريقة أبسط لتنظيم شغلهم
            ومتابعة حساباتهم.
          </p>
        </div>
...
export function Testimonial({
  ...
  return (
    <article
      className={`group/card relative overflow-hidden rounded-2xl bg-gray-800 p-px transition-opacity ${!testimonial.categories.includes(category) ? "opacity-30" : ""}`}
    >
      <div className="relative z-20 flex h-full flex-col gap-4 rounded-[inherit] bg-gray-950 p-5">
        <CardScene kind={testimonial.scene} />
        <p className="text-gray-200 before:content-['“'] after:content-['”']">
          {children}
        </p>
